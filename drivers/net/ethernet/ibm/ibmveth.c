...
static void reuse_skb(struct sk_buff *skb) {
	...
	netdev_dbg(adapter->netdev, "to unalloc: %llu diff_addr: %d\n", correlator, 
			adapter->rx_buff_pool[correlator >> 32].dma_map[correlator & 0xffffffffUL].addr 
				- (void *)skb->data );
...
static inline struct sk_buff *ibmveth_rxq_get_buffer(struct ibmveth_adapter *adapter)
{
	...
	*(u64 *)&adapter->rx_buff_pool[pool].skbuff[index]->cb[40] = correlator;
	netdev_dbg(adapter->netdev, "%llu data_len = %d\n", correlator, adapter->rx_buff_pool[pool].skbuff[index]->data_len);
